...
class Database
{
  ...
  public function __construct($db_name = false, string $db_host = 'localhost', string $db_user = 'root', string $db_pass = 'root')
  {
    if (gettype($db_name) === 'string')
    {
    ...
    {
      $logs = $this->getConfig();
      $this->db_name = $logs->name;
      $this->db_host = $logs->host;
      $this->db_user = $logs->user;
      $this->db_pass = $logs->password;
    }
    else die('<pre>Error ~ incorrect database information</pre>');
  }
